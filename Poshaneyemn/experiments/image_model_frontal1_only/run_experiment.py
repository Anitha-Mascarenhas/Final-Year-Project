...
SPLIT_CHILDREN_PATH = (
    ROOT / "image_model_binary_child_level" / "artifacts" / "split_children.json"
)
...
def main():
    ...
    split_children = json.loads(SPLIT_CHILDREN_PATH.read_text(encoding="utf-8-sig"))

    train_items = manifest["splits"]["train"]["items"]
    val_items = manifest["splits"]["validation"]["items"]
    test_items = manifest["splits"]["test"]["items"]
    ...
    print(f"train children={len(train_items)} images={len(train_paths)}")
    print(f"validation children={len(val_items)} images={len(val_paths)}")
    print(f"test children={len(test_items)} images={len(test_paths)}")
    print(
    ...
    out = {
        ...
        "split": {
            "train_children": len(train_items),
            "validation_children": len(val_items),
            "test_children": len(test_items),
        ...
        "test_subgroups": {
            tag: int(label) for tag, label in zip(test_tags, test_labels)
        },
    }
