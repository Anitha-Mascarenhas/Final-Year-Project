...
# Confirm mutual exclusivity of splits (by image_path if available)

def _overlap(a, b, col='image_path'):
    if col in a.columns and col in b.columns:
        return len(set(a[col].astype(str)).intersection(set(b[col].astype(str))))
    return 0

print('OVERLAP_TRAIN_VALID:', _overlap(train, val))
print('OVERLAP_TRAIN_TEST :', _overlap(train, test))
print('OVERLAP_VALID_TEST :', _overlap(val, test))
