...
class AndroidCameraCameraX extends CameraPlatform {
  ...
  /// PoshanEye patch: whether a frame stream was requested by the app. When
  /// false, initializeCamera skips instantiating/binding ImageAnalysis so the
  /// device only needs to support Preview + ImageCapture.
  bool _imageOutputFormatRequested = false;
  ...
  Future<void> initializeCamera(
    ...
    // PoshanEye patch (see third_party/README.md): ImageAnalysis is only
    // instantiated when a frame stream is actually requested. On several
    // devices (e.g. OPPO CPH2603 / ColorOS) binding Preview + ImageCapture +
    // ImageAnalysis simultaneously exceeds the camera's supported surface
    ...
    if (_imageOutputFormatRequested) {
      imageAnalysis = ImageAnalysis(
        resolutionSelector: _presetResolutionSelector,
        targetFpsRange: _targetFpsRange,
    ...
    camera = await processCameraProvider!.bindToLifecycle(
      ...
      <UseCase>[
        preview!,
        imageCapture!,
        if (imageAnalysis != null) imageAnalysis!,
      ],
    );
  ...
  Stream<CameraImageData> onStreamedFrameAvailable(
    ...
    // PoshanEye patch: remember that a frame stream was actually requested so
    // initializeCamera binds the ImageAnalysis use case only in that case.
    _imageOutputFormatRequested = true;
